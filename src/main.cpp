...
void llcom_helper(string error_msg, char* argv[]) {
    cerr << ERROR << error_msg << endl << endl;
    cerr << PINK << "LiteLang Compiler (llcom) has 2 options: " << endl;
    cerr << PINK << "Usage: " << argv[0] << " <input.cal>" << " run" << endl;
    cerr << PINK << "See Code: " << argv[0] << " <input.cal>" << " code" << endl;
    exit(EXIT_FAILURE);
}

int main(int argc, char* argv[]) {
  if (argc < 3) {
    llcom_helper("[Invalid Compile]", argv);
  }
  ...
  string filename = argv[1];
    ...
    if (!hasExtension(filename)) {
        llcom_helper("[Invalid File]", argv);
    ...
    if (string(argv[2]) == "code") {
        ...
        cout << PINK << "LiteLang Compiler (llcom) has 2 options: " << endl;
        cout << PINK << "Usage: " << argv[0] << " <input.cal>" << " run" << endl;
        cout << PINK << "See Code: " << argv[0] << " <input.cal>" << " code" << endl;
        exit(EXIT_SUCCESS);
    }
    else {
    ...
    cout << GREEN << "[Compile Success] ./out is ready" << endl;
